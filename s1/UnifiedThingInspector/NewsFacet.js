...
/*	var oHourAgo = new Date();
	...
	oNewsFacetContent = new sap.suite.ui.commons.FeedTile("feedtile_news", {
		items: [oArticleNews1,oArticleNews2,oArticleNews3],
		displayDuration: 5
	});*/
	oNewsFacetContent = new sap.m.Carousel("carousel_news", {
		...
		pages: [
		        new sap.m.Image({ src: "images/news1.jpg", width: "100%", height: "100%"}),
		        new sap.m.Image({ src: "images/news2.jpg", width: "100%", height: "100%"}),
		        new sap.m.Image({ src: "images/news3.jpg", width: "100%", height: "100%"})
		        ]
	}).addDelegate({
	...
	oNewsFacetContent = new sap.m.SlideTile({
		transitionTime: 500,
		tiles: [
...
var oNewsFacet = new sap.suite.ui.commons.FacetOverview("facet_news", {
    title: "新鲜推荐",
    quantity: 3,
    content: isPhone ? sap.m.HBox({
    	justifyContent: "Center",
    	items: oNewsFacetContent
    }).addStyleClass("sapUiContentPadding") : oNewsFacetContent,
