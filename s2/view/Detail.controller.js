...
sap.ui.controller("sap.ui.sample.poa.view.Detail", {
	...
	_handleRouteMatched: function(evt){
		...
		var sPath = sap.ui.sample.poa.util.objectSearch.getPath(oModel.getData(), "ID", this._detailId);
		if(!sPath){
			this._oRouter.myNavToWithoutHash("sap.ui.sample.poa.view.NotFound", "XML", false, {path: this._detailId});
			return;
		}
	...
	_refresh : function (channelId, eventId, data) {
		if (data && data.context) {
			...
			/*
			 * scrollTo(iY, iTime) : sap.m.Page
             * Scrolls to the given position. Only available if enableScrolling is set to "true".
			 * Parameters:
			 *  {int} iY	 The vertical pixel position to scroll to. Scrolling down happens with positive values.
			 *  {int} iTime	 The duration of animated scrolling. To scroll immediately without animation, 
			 *  			 give 0 as value. 0 is also the default value, when this optional parameter is omitted. 
			 * */
		}
	},
	...
	handleTabSelect : function (evt) {
		...
		if (item.getContent().length === 0) {
			var view = new sap.ui.view({
				id : "tabView" + key,
				viewName : "sap.ui.sample.poa.view.Detail" + key,
				type : "XML"
			});
	...
	_createRecipientDialog : function (evt) {
		...
		this._recipientDialog = sap.ui.xmlfragment("sap.ui.sample.poa.view.RecipientHelpDialog", this);
